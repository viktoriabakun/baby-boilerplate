...
body {
  ...
  color: white; // I hope nobody writes like this, hehe
}
...
Magic numbers dramatically hurt code maintainability and should be avoided at all time. When in doubt, extensively
explain the questionable value.

> In this example, instead of directly using the magic number 960px in the styles, we've assigned it to a named constant
> $container-width. This way, if the width needs to be changed later, you only need to update the value of the constant,
